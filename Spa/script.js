...
let currentItem = null; // Şu an görüntülenen öğe için global değişken
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    // Splash screen'i doğrudan gizle ve main content'i göster
    const splashScreen = document.getElementById('splash-screen');
    const mainContent = document.getElementById('main-content');
    
    if (splashScreen) {
        splashScreen.style.display = 'none'; // Splash screen'i gizle
    }
    
    if (mainContent) {
        mainContent.style.display = 'block'; // Main content'i göster
    }
    ...
    fetch(jsonFilePath)  // Doğru dosyayı yükle
      ...
      .then(data => {
        ...
        data.menu.forEach(category => {
          ...
          // Açıklama metni artık kart içinde gösterilmeyecek
          // Bunun yerine gizli bir tag veya veri özniteliği olarak saklanacak
          categoryCard.dataset.description = category.description || '';

          const button = document.createElement('button');
          button.textContent = 'Choose';
          button.addEventListener('click', () => {
            // Açıklama metnini popup ile göster
            showDescriptionPopup(category);
          });
...
function showCategoryItems(category) {
  ...
  category.items.forEach(item => {
    ...
    // Açıklama metni artık kart içinde gösterilmeyecek
    // Bunun yerine gizli veri özniteliği olarak saklanacak
    ...
    // Aliva Spa kategorisindeki öğeler için Choose butonu gösterme
    if (category.name !== "Aliva Spa") {
      const chooseButton = document.createElement('button');
      chooseButton.textContent = 'Choose';
      chooseButton.addEventListener('click', () => {
        // Açıklama için popup göster
...
// Açıklama popup'ını gösteren fonksiyon - "Açıklama bulunmuyor" metnini kaldırıyoruz
function showDescriptionPopup(item) {
  currentItem = item; // Şu anki öğeyi global değişkene ata
  
  // Mevcut popup varsa temizle
  const existingPopup = document.getElementById('description-popup-overlay');
  if (existingPopup) {
  ...
  popupOverlay.id = 'description-popup-overlay';
  popupOverlay.className = 'description-popup-overlay';
  
  // Popup içeriği oluştur
  const popupContent = document.createElement('div');
  popupContent.className = 'description-popup';
  
  // Kapatma ikonu
  const closeIcon = document.createElement('button');
  closeIcon.className = 'description-popup-close-icon';
  closeIcon.innerHTML = '&times;';
  closeIcon.addEventListener('click', closeDescriptionPopup);
  
  // Başlık
  const header = document.createElement('div');
  header.className = 'description-popup-header';
  
  const title = document.createElement('h3');
  title.className = 'description-popup-title';
  ...
  description.className = 'description-popup-content';
  description.textContent = item.description || ''; // Açıklama yoksa boş string göster
  
  // Butonlar
  const footer = document.createElement('div');
  footer.className = 'description-popup-footer';
  
  // Dil çevirilerini kullan
  const closeText = translations[currentLanguage]?.close || 'Kapat';
  const continueText = translations[currentLanguage]?.continue || 'Devam Et';
  
  const closeButton = document.createElement('button');
  closeButton.className = 'description-popup-close';
  closeButton.textContent = closeText;
  closeButton.addEventListener('click', closeDescriptionPopup);
  
  // Alt kategorileri kontrol et
  if (item.items && Array.isArray(item.items) && item.items.length > 0) {
    // Eğer alt öğeler varsa, devam et butonu alt listeyi göstersin
    const continueButton = document.createElement('button');
    continueButton.className = 'description-popup-continue';
    ...
    // Alt öğeler yoksa, bu son kategoridir, sepete ekle
    const continueButton = document.createElement('button');
    continueButton.className = 'description-popup-continue';
    continueButton.textContent = continueText;
    continueButton.addEventListener('click', () => {
      // Fiyat değeri yoksa otomatik olarak ekle
...
// Açıklama popup'ını kapatan fonksiyon
function closeDescriptionPopup() {
  const popup = document.getElementById('description-popup-overlay');
  if (popup) {
    popup.remove();
  }
  currentItem = null; // Global değişkeni temizle
}
